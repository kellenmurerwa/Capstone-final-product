...
**Evaluation protocol:** temporal hold-out — train 2018–2023, test on the unseen **2024** year
...
| 3 | Benchmark ≥3 supervised models vs rainfall-threshold & static-polygon baselines | **macro-F1 ≥ 0.75** and **high-recall ≥ 0.80** | RF **0.831 / 0.867**; XGBoost **0.813 / 0.847** | ✅ Met |
| 4 | Integrate HMM; evaluate next-step accuracy + spatial concordance with official polygons | **enrichment lift ≥ 1.4** and **inside/outside odds ≥ 1.5** | lift **1.59×**, odds **1.84×**; HMM next-step acc 0.445 | ✅ Met (spatial) / ⚠️ partial (HMM) |
...
|---|---|---|---|---|
| **Random Forest** | **0.831** | 0.847 | **0.867** | Best overall |
| XGBoost (+SHAP) | 0.813 | 0.833 | 0.847 | Close second; explainable |
| Decision Tree | 0.753 | 0.783 | 0.793 | Just clears F1 bar |
| Logistic Regression | 0.749 | 0.781 | 0.821 | Linear baseline |
| Rainfall-threshold baseline | 0.623 | 0.637 | 0.902 | High recall, poor precision |
...
and high-recall ≥ 0.80 bars on a *temporal* hold-out — the honest test, because
2024 is never seen in training. The Random Forest improves macro-F1 by **+0.21
absolute (+33%)** over the rainfall-threshold baseline and by **+0.51** over the
static-polygon map. This directly answers Research Question 3: a geospatial ML
classifier separates Low/Moderate/High pressure far better than either rule-based
baseline.

**Why this matters for the use-case.** High-pressure **recall** (0.867) was
weighted above precision in the proposal because a missed high-pressure day is
costlier than a false alarm. The model recovers ~87% of true high-pressure
...
|---|---|---|
| Enrichment / lift = P(official zone \| predicted-High) / P(official zone) | ≥ 1.4 | **1.59×** ✅ |
| Inside-vs-outside High odds ratio | ≥ 1.5 | **1.84×** ✅ |
...
model concentrates High-pressure predictions **1.84× more densely inside** the
